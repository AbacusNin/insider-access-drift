...
The events come from a generator (`generate`), not from real logs. There is no public dataset of labeled insider access, and real access logs are not something you put in a public repo. So the tool ships its own: a fixed-seed synthetic log of a few benign peer groups doing normal work, plus three planted bad actors whose behavior is known ahead of time (a slow accumulator, an after-hours crown-jewel puller, and a contractor reaching across too many restricted resources). Because the three are planted, the tests and CI can assert the scorer flags exactly them and nobody else. Real logs have no labels, so you cannot run that check there. The seed is fixed, so every run produces the same log and a result is reproducible. Point the tool at your own schema-shaped CSV and the generator drops out of the picture.
